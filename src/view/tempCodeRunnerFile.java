    ...
    private void actualizarBotonesEnemigos() {
        ...
        for (Enemigo e : enemigos) {
            if (e.estaVivo()) {
                ...
                nombreEnemigo.setForeground(new Color(60, 30, 10));

                // ----------- CARTEL (PIXEART) ABAJO ----------
                JPanel fondoPergamino = new JPanel(new BorderLayout()) {
                    @Override
                    protected void paintComponent(Graphics g) {
                        super.paintComponent(g);
                        Image img = new ImageIcon(getClass().getResource("/img/Pergamino.png")).getImage();
                        g.drawImage(img, 0, 0, getWidth(), getHeight(), this);
                    }
                };
                fondoPergamino.setOpaque(false);
                fondoPergamino.setPreferredSize(new Dimension(130, 35));

                // ORDEN NUEVO: nombre → imagen → cartel
                contenedor.add(nombreEnemigo, BorderLayout.NORTH);
                contenedor.add(btn, BorderLayout.CENTER);
                contenedor.add(fondoPergamino, BorderLayout.SOUTH);
